...
static void	init_camera_vectors(t_cub *m)
{
	...
	c = &m->camera;
	c->viewport_u = vec4_init(m->viewport_w, 0, 0, 0);
	c->viewport_v = vec4_init(0, -m->viewport_h, 0, 0);
	v = vec4_init(m->viewport_w / 2, -m->viewport_h / 2, m->focal_length, 0);
	c->upper_left_corner = vec4_sub(c->center, v);
	c->pixel_delta_u = vec4_scale(c->viewport_u, 1.0 / m->width);
	c->pixel_delta_v = vec4_scale(c->viewport_v, 1.0 / m->height);
	c->pixel00_loc = vec4_add(c->upper_left_corner,
			vec4_scale(vec4_add(c->pixel_delta_u, c->pixel_delta_v),
				0.5));
}
...
//     t_vec4 up = vec4_init(0, 1, 0, 0);
//     // si axis parallèle à up, choisir un autre up
//     if (fabs(vec3_dot(c->axis, up)) > 0.999)
//         up = vec4_init(1, 0, 0, 0);

//     // Calcul des axes locaux de la caméra
//     t_vec4 w = vec4_scale(c->axis, -1);          // backward
//     t_vec4 u = vec3_normalize(vec3_cross(up, w)); // droite
//     t_vec4 v = vec3_cross(w, u);                 // haut
...
//     c->viewport_u = vec4_scale(u, viewport_w);
//     c->viewport_v = vec4_scale(v, viewport_h);

//     // Coin supérieur gauche du viewport
//     t_vec4 half_u = vec4_scale(u, viewport_w / 2.0);
//     t_vec4 half_v = vec4_scale(v, viewport_h / 2.0);
//     c->upper_left_corner = vec4_sub(
//         vec4_add(c->center, vec4_scale(c->axis, m->focal_length)), // avancer vers la scène
//         vec4_add(half_u, half_v)                                  // reculer à gauche et en haut
//     );

//     // Delta pour parcourir pixel par pixel
//     c->pixel_delta_u = vec4_scale(c->viewport_u, 1.0 / m->width);
//     c->pixel_delta_v = vec4_scale(c->viewport_v, -1.0 / m->height); // - pour descendre

//     // Position du pixel (0,0) au centre du pixel
//     c->pixel00_loc = vec4_add(c->upper_left_corner,
//         vec4_scale(vec4_add(c->pixel_delta_u, c->pixel_delta_v), 0.5));
// }
